...
const HEADER_CORS_REQUEST_METHOD = 'access-control-request-method';
const HEADER_CORS_REQUEST_HEADERS = 'access-control-request-headers';
const HEADER_CORS_ALLOW_ORIGIN = 'access-control-allow-origin';
const HEADER_CORS_ALLOW_METHODS = 'access-control-allow-methods';
const HEADER_CORS_ALLOW_HEADERS = 'access-control-allow-headers';
const HEADER_CORS_ALLOW_CREDENTIALS = 'access-control-allow-credentials';
const HEADER_CORS_EXPOSE_HEADERS = 'access-control-expose-headers';
...
function buildOriginHandler(_config) {
  if (_config === true) {
    return async s => s.out[HEADER_CORS_ALLOW_ORIGIN] = s.in[HEADER_ORIGIN];
  } else if (typeof _config === 'string') {
    return async s => {
      s.out[HEADER_CORS_ALLOW_ORIGIN] = _config;
      s.out.appendUnique(HEADER_VARY, HEADER_ORIGIN);
    };
    ...
    return async s => {
      ...
      if (await asyncSome(tests, t => t(origin))) {
        s.out[HEADER_CORS_ALLOW_ORIGIN] = origin;
      } else {
        s.out[HEADER_CORS_ALLOW_ORIGIN] = false;
      }
...
function buildMethodsHandler(_config) {
  if (typeof _config === 'string') {
    return s => s.out[HEADER_CORS_ALLOW_METHODS] = _config;
  } else if (Array.isArray(_config)) {
    return s => s.out[HEADER_CORS_ALLOW_METHODS] = _config.join(',');
  }

  return s => s.out[HEADER_CORS_ALLOW_METHODS] = s.in[HEADER_CORS_REQUEST_METHOD];
}

function buildAllowedHeadersHandler(_config) {
  if (typeof _config === 'string') {
    return s => s.out[HEADER_CORS_ALLOW_HEADERS] = _config;
  } else if (Array.isArray(_config)) {
    return s => s.out[HEADER_CORS_ALLOW_HEADERS] = _config.join(',');
  }

  return s => s.out[HEADER_CORS_ALLOW_HEADERS] = s.in[HEADER_CORS_REQUEST_HEADERS];
}

function buildExposedHeadersHandler(_config) {
  if (typeof _config === 'string') {
    return s => s.out[HEADER_CORS_EXPOSE_HEADERS] = _config;
  } else if (Array.isArray(_config)) {
    return s => s.out[HEADER_CORS_EXPOSE_HEADERS] = _config.join(',');
  }
...
function buildCredentialsHandler(_config) {
  if (_config === true) {
    return s => s.out[HEADER_CORS_ALLOW_CREDENTIALS] = true;
  }
...
function buildMaxAgeHandler(_config) {
  if (Number.isInteger(_config)) {
    return s => s.out[HEADER_CORS_MAX_AGE] = _config;
  }
