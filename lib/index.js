...
function buildOriginHandler(_config) {
  if (_config === true) {
    return async s => s.out[http2.constants.HTTP2_HEADER_ACCESS_CONTROL_ALLOW_ORIGIN] = s.in[HEADER_ORIGIN];
  } else if (typeof _config === 'string') {
    return async s => {
      s.out[http2.constants.HTTP2_HEADER_ACCESS_CONTROL_ALLOW_ORIGIN] = _config;
...
function buildAllowedHeadersHandler(_config) {
  ...
  return s => s.out[http2.constants.HTTP2_HEADER_ACCESS_CONTROL_ALLOW_HEADERS]
    = s.in[http2.constants.HTTP2_HEADER_ACCESS_CONTROL_REQUEST_HEADERS];
}
